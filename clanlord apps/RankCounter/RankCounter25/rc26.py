...
# ---------------- Characters Tab ----------------
character_list = tk.Listbox(frame_characters, height=10, width=40)
...
def remove_character():
    ...
    if messagebox.askyesno("Confirm", f"Remove character '{name}'?"):
        ...
        # Update dropdown
        new_values = list(character_list.get(0, tk.END))
        character_dropdown['values'] = new_values

        # Select first character if any remain
        if new_values:
            character_dropdown.current(0)
            selected_character.set(new_values[0])
            on_character_change()
        else:
...
character_dropdown['values'] = character_list.get(0, tk.END)
character_dropdown.current(0)
...
def on_character_change(event=None):
    name = selected_character.get()
    folder_list.delete(0, tk.END)
    for f in character_folders.get(name, []):
        folder_list.insert(tk.END, f)

character_dropdown.bind("<<ComboboxSelected>>", on_character_change)
...
tk.Button(frame_logsearch, text="Open File", command=ls_open_selected_file)\
